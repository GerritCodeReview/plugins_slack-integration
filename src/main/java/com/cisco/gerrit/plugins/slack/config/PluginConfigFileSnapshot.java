...
public class PluginConfigFileSnapshot implements PluginConfigSnapshot
{
    ...
    private boolean publishOnChangeMerged = false;
    private boolean publishOnCommentAdded = false;
    private boolean publishOnPatchSetCreated = false;
    private boolean publishOnReviewerAdded = false;
    		...
    		String gitBranch)
    {
        ...
        try
        {
            ...
            if (subsections.contains(gitBranch))
            {
            	...
            	publishOnChangeMerged = pluginConfig.getBoolean(
                        BRANCH_SECTION_NAME, gitBranch, "publish-on-reviewer-added", IRRELEVANT_VALUE);
            	publishOnPatchSetCreated = pluginConfig.getBoolean(
                        BRANCH_SECTION_NAME, gitBranch, "publish-on-patch-set-created", IRRELEVANT_VALUE);
            	publishOnCommentAdded = pluginConfig.getBoolean(
                        BRANCH_SECTION_NAME, gitBranch, "publish-on-comment-added", IRRELEVANT_VALUE);
            	publishOnReviewerAdded = pluginConfig.getBoolean(
                        BRANCH_SECTION_NAME, gitBranch, "publish-on-reviewer-added", IRRELEVANT_VALUE);
            }
        }
