...
public class TestToolbox
{
			...
			String projectName) throws Exception
	{
        ...
        when(pluginConfig.getString("ignore", ""))
                .thenReturn("^WIP.*");
        when(pluginConfig.getBoolean("publish-on-patch-set-created", true))
                .thenReturn(true);
        when(pluginConfig.getBoolean("publish-on-change-merged", true))
                .thenReturn(true);
        when(pluginConfig.getBoolean("publish-on-comment-added", true))
                .thenReturn(true);
        when(pluginConfig.getBoolean("publish-on-reviewer-added", true))
                .thenReturn(true);
			...
			String branchName) throws Exception
	{
		...
		when(pluginConfig.getString(
			...
			.thenReturn("^WIP.*");
        when(pluginConfig.getBoolean(
                PluginConfigFileSnapshot.BRANCH_SECTION_CONFIG_NAME, branchName, "publish-on-patch-set-created", false))
            .thenReturn(true);
